...
namespace sk {
	namespace skprintf {
		int [[gnu::fastcall]] [[msvc::__fastcall]] localPrintString(FILE* buffer, const char* str) {
			int written = 0;
		...
		int [[gnu::fastcall]] [[msvc::__fastcall]] printBase(int base, FILE* buffer = stdout) {
			switch (base) {
			case 16:
		...
		int [[gnu::fastcall]] [[msvc::__fastcall]] getBase(char formatc) {
			switch (formatc) {
			case 'b':
				return 2;
			case 'o':
		...
		int [[gnu::fastcall]] [[msvc::__fastcall]] localPuts(const char* str, FILE* buffer = stdin) {
			return localPrintString(buffer, str);
		}

		int [[gnu::fastcall]] [[msvc::__fastcall]] localvfprintf(FILE* buffer, const char* format, va_list* args) {
			int base, written = 0;
			char numBuffer[65];
